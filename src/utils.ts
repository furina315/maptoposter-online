  ...
  return {
    ...
    features: geojson.features.map((feature) => {
      ...
      return {
        ...feature,
        geometry,
        properties: keepProperties.reduce(
          (acc, key) => {
...
export async function fetchFromProtomaps(
  ...
  console.log(
    `[Protomaps] Download complete. Roads: ${roads.length}, Water: ${water.length}, Parks: ${landuse.length}, POIs: ${pois.length}`
  );
...
export async function fetchFeatures(
  ...
  const tagFilters = Object.entries(tags)
    .flatMap(([key, value]) => {
      ...
      return [`way${filter}(${bbox});`, `relation${filter}(${bbox});`];
    })
    .join("");
